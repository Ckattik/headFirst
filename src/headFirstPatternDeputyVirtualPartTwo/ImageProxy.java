...
public class ImageProxy implements Icon{
	...
	ImageIcon imageIcon;                // в переменной храниться НАСТОЯЩИЙ обьект Icon который должен отображаться после загрузки   
	URL imageURL;                        
	Thread retrievalThread;
	boolean retrieving = false;
	
	
	public ImageProxy(URL url) {                // Конструктору передаеться URL адрес изображения - того, которое должно отображаться после загрузки 
	...
	@Override
	public int getIconHeight() {                      // До завершения загрузки возвращаються значения длины и ширыны по умолчанию; 
